...
export function Age({at}:{at:string}){const now=useClock();if(!now)return <span>—</span>;const seconds=Math.max(0,Math.floor((now-Date.parse(at))/1000));return <span title={at}>{seconds<60?`${seconds}s`:seconds<3600?`${Math.floor(seconds/60)}m ${seconds%60}s`:`${Math.floor(seconds/3600)}h ${Math.floor(seconds%3600/60)}m`}</span>;}
export function Countdown({deadline,compact=false}:{deadline:string;compact?:boolean}){
 const now=useClock();if(now===null)return <span>CALCULATING…</span>;
...
export function LifeState({life}:{life:Lifecycle}){return life.deadline?<Countdown deadline={life.deadline} compact/>:<span className="muted">NONE ON FILE</span>;}
